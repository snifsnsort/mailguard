...
class DnsInfo:
    ...
    def is_registered(self) -> bool:
        return self.has_a or self.has_aaaa or self.has_mx
...
async def _resolve_domain(domain: str, semaphore: asyncio.Semaphore) -> DnsInfo:
    ...
    async with semaphore:
        ...
        try:
            ...
            info.aaaa_records = [str(r) for r in ans]
            info.has_aaaa = True
        except Exception:
            pass
        ...
        try:
            ...
            info.ns_records = [str(r).rstrip(".") for r in ans]
            info.has_ns = True
        except Exception:
            pass

        # TXT records (first 3 only — can be noisy)
        try:
            ans = await resolver.resolve(domain, "TXT")
            info.txt_records = [b.decode() for r in ans for b in r.strings][:3]
            info.has_txt = bool(info.txt_records)
        except Exception:
            pass
...
class WhoisInfo:
    domain:           str
    registered_date:  Optional[str]  = None   # ISO string
...
async def fetch_rdap(domain: str, timeout: float = 6.0) -> Optional[WhoisInfo]:
    """
    Fetch RDAP data for a domain using rdap.org (free, no auth required).
    Returns WhoisInfo or None on any failure.
    """
    import httpx
        ...
        if isinstance(vcard, list) and len(vcard) > 1:
            for prop in vcard[1]:
                if isinstance(prop, list):
                    if prop[0] == "fn":
                        name = prop[3] if len(prop) > 3 else None
                    elif prop[0] == "email":
                        email = prop[3] if len(prop) > 3 else None
...
async def fetch_ct_crtsh(domain: str, timeout: float = 8.0) -> List[CertInfo]:
    """
    Query crt.sh for recently issued certificates for a domain.
    Free, no auth required. Returns up to 10 most recent certs.
    """
    import httpx
...
# Cloud service patterns found in NS or MX records that indicate
# the domain is hosted on potentially claimable infrastructure
...
def detect_subdomain_takeover(dns_info: DnsInfo) -> Optional[str]:
    """
    Check NS and MX records for patterns suggesting the domain is hosted on
    cloud infrastructure that may be claimable (takeover risk).
    Returns service name if risk detected, None otherwise.
    """
    all_records = (
        [r.lower() for r in dns_info.ns_records] +
        [r.lower() for r in dns_info.mx_records]
    )
...
class AggressiveResult:
    """Fully enriched lookalike result from the aggressive scanner."""
    candidate:        str
    base_domain:      str
    similarity_score: int            # raw similarity score 0-100
    enriched_score:   int            # after DNS/age/CT bonuses 0-100
    ...
    takeover_risk:    Optional[str]           = None


def _infer_mutation_type(match: LookalikMatch) -> str:
    """Infer the dominant mutation category from a scored LookalikMatch."""
    if match.has_homoglyphs and match.mixed_script:
        return "unicode_homoglyph_mixed_script"
    if match.has_homoglyphs:
        return "unicode_homoglyph"
    if match.tld_swap:
        return "tld_substitution"

    _, base_label, _ = normalize_domain(match.base_domain)
    _, cand_label, _ = normalize_domain(match.candidate)
    base_norm, _, _  = strip_homoglyphs(base_label)
    cand_norm, _, _  = strip_homoglyphs(cand_label)

    lev = levenshtein(base_norm, cand_norm)
    dam = damerau_levenshtein(base_norm, cand_norm)

    if dam < lev and dam <= 2:
        return "transposition"
    if match.signals.keyboard_typo >= 0.6:
        return "keyboard_substitution"
    if lev == 1:
        if len(base_norm) > len(cand_norm):
            return "character_omission"
        elif len(base_norm) < len(cand_norm):
            return "character_insertion"
        else:
            return "character_substitution"
    if "-" in cand_norm and "-" not in base_norm:
        return "hyphenation_attack"
    if lev <= 3:
        return "multi_character_edit"
...
def compute_enriched_score(
    ...
    certs: List[CertInfo],
    takeover_risk: Optional[str],
) -> Tuple[int, str]:
    """
    Apply enrichment bonuses/penalties to the similarity score.
    Rules:
    - No real signals → always low
    - Domain age ≤ 365 days → critical boost (+20)
    - Domain age 366–1095 days (1–3 years) → moderate boost (+8)
    - Domain age > 1095 days (>3 years) → penalty (−15, cap at low)
    - MX present → +15 (strongest phishing signal)
    - A/AAAA present → +8
    - CT hit → +8
    - Takeover risk → +12
    Returns (enriched_score 0-100, risk_level string).
    """
    ...
    if dns_info:
        if dns_info.has_mx:
            score += 15
            has_any_signal = True
        elif dns_info.has_a or dns_info.has_aaaa:
            score += 8
            has_any_signal = True

    age_days = whois_info.age_days if (whois_info and whois_info.age_days is not None) else None

    if age_days is not None:
        if age_days <= 365:
            # Registered in last year — strong phishing indicator
            score += 20
            has_any_signal = True
        elif age_days <= 1095:
            # 1–3 years old — moderate concern
            score += 8
        else:
            # Older than 3 years — likely benign, penalise
            score -= 15
    ...
    # No active signals at all → always low regardless of similarity score
    if not has_any_signal:
        return score, "low"

    # Old domain (>3 years) → cap at low
    if age_days is not None and age_days > 1095 and not (dns_info and dns_info.has_mx) and not certs:
        return min(score, 34), "low"

 ...
 *
 * Displays authentication health scan results for a domain:
 * SPF, DKIM, DMARC posture + security findings.
 *
 * API: GET /api/v2/authentication/{domain}
 */
...
interface DkimEvidence {
  ...
  selectors_attempted: number
  providers: string[]
}

interface ScoreBreakdown {
  spf_exposure: number
  dmarc_exposure: number
  dkim_exposure: number
  cross_exposure: number
  total: number
}
...
function ProtoBadge({
  label, present, policy, selectors,
}: {
...
function FindingRow({ f }: { f: Finding }) {
  ...
  return (
    <div style={{
      border: `1px solid ${s.border}`, borderRadius: 8, marginBottom: 6, overflow: 'hidden',
    }}>
      <div
        onClick={() => setOpen(v => !v)}
        style={{
      ...
      >
        <span style={{
          ...
          color: s.color, flexShrink: 0, marginTop: 1, minWidth: 58,
        }}>{f.severity}</span>
...
export default function AuthHealthPage() {
  ...
  const handleScan = async () => {
    ...
    setResult(null)
    try {
      const res = await fetch(`/api/v2/authentication/${encodeURIComponent(d)}`)
      if (!res.ok) {
  ...
  // Filter out the lead summary finding for the detail sections
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        background: 'rgba(8,12,18,0.85)', backdropFilter: 'blur(10px)',
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
            Evaluate email authentication posture and spoofing risk for any domain
          </p>
        </div>
      </div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 860 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 28 }}>
          ...
          <button
            ...
            style={{
              ...
              background: loading || !domain.trim() ? 'var(--surface2)' : 'var(--accent)',
              color: loading || !domain.trim() ? 'var(--muted)' : '#000',
              border: 'none', cursor: loading || !domain.trim() ? 'not-allowed' : 'pointer',
              fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-body)', transition: 'all .15s',
            }}
          >
        ...
        {result && ev && (
          <>
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 12 }}>
                  ...
                  {critical === 0 && high === 0 && (
                    <span style={{ color: '#22c55e', marginLeft: 8 }}>· no critical or high issues</span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
                  ...
                  <ProtoBadge label="DKIM"  present={ev.dkim.selectors_found.length > 0}
                    selectors={ev.dkim.selectors_found.length} />
                </div>
              ...
              {ev.spf.present ? (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 32px' }}>
                    <KV label="Policy" value={POLICY_LABEL[ev.spf.policy ?? ''] ?? ev.spf.policy ?? '—'}
                      valueColor={POLICY_COLOR[ev.spf.policy ?? '']} />
                    <KV label="DNS Lookups" value={`${ev.spf.lookup_count} / 10`}
                      valueColor={ev.spf.lookup_count > 10 ? '#ef4444' : ev.spf.lookup_count >= 9 ? '#f97316' : '#22c55e'} />
                    <KV label="Multiple records" value={ev.spf.multiple ? 'Yes ⚠' : 'No'}
                      valueColor={ev.spf.multiple ? '#ef4444' : '#22c55e'} />
            ...
            <Collapsible title="DMARC" icon="🛡" defaultOpen={ev.dmarc.present}>
              {ev.dmarc.present ? (
                <>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 32px' }}>
                    <KV label="Policy (p=)" value={ev.dmarc.policy ?? '—'}
                      valueColor={POLICY_COLOR[ev.dmarc.policy ?? '']} />
                    <KV label="Subdomain (sp=)" value={ev.dmarc.subdomain_policy ?? 'inherits p='}
                      valueColor={POLICY_COLOR[ev.dmarc.subdomain_policy ?? '']} />
                    <KV label="Enforcement pct" value={`${ev.dmarc.pct}%`}
                      valueColor={ev.dmarc.pct === 100 ? '#22c55e' : '#f97316'} />
                    ...
                    <KV label="Multiple records" value={ev.dmarc.multiple ? 'Yes ⚠' : 'No'}
                      valueColor={ev.dmarc.multiple ? '#ef4444' : '#22c55e'} />
                  </div>
                ...
                <div style={{ color: '#ef4444', fontSize: 13 }}>
                  No DMARC record found at _dmarc.{ev.domain}
                </div>
              )}
            </Collapsible>
            ...
            {/* DKIM detail */}
            <Collapsible
              title={`DKIM (${ev.dkim.selectors_found.length} selector${ev.dkim.selectors_found.length !== 1 ? 's' : ''} found)`}
              icon="🔑"
              defaultOpen={ev.dkim.selectors_found.length > 0}
            >
              {ev.dkim.selectors_found.length > 0 ? (
                <>
                  {ev.dkim.selectors_found.map((s: DkimSelector) => (
                    <div key={s.selector} style={{
                      marginBottom: 10, padding: '10px 14px',
                      background: 'rgba(0,229,255,0.03)', border: '1px solid var(--border)', borderRadius: 8,
                    }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        {s.key_bits !== undefined && (
                          <span style={{
                            padding: '2px 8px', borderRadius: 10, fontSize: 10,
                            background: s.key_bits >= 2048 ? 'rgba(34,197,94,0.08)' : 'rgba(234,179,8,0.08)',
                            border: `1px solid ${s.key_bits >= 2048 ? 'rgba(34,197,94,0.3)' : 'rgba(234,179,8,0.3)'}`,
                            color: s.key_bits >= 2048 ? '#22c55e' : '#eab308',
                          }}>~{s.key_bits} bits</span>
                        )}
                      </div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
                    Checked {ev.dkim.selectors_attempted} common selectors
                  </div>
                </>
              ) : (
                <div style={{ color: 'var(--muted)', fontSize: 13 }}>
                  No DKIM selectors found after checking {ev.dkim.selectors_attempted} common names.
                  DKIM may use a non-standard selector — verify manually.
                </div>
              )}
            </Collapsible>
            ...
            <Collapsible title="Raw DNS Evidence" icon="🗂">
              ...
              <div style={{ marginTop: 8, fontSize: 11, color: 'var(--muted)' }}>
                Scan completed in {ev.scan_metadata.scan_duration_ms}ms ·{' '}
                Module {ev.scan_metadata.module_version} ·{' '}
                {ev.scan_metadata.selectors_checked} DKIM selectors checked
              </div>
            </Collapsible>
          </>
        )}
        ...
        {!result && !loading && !error && (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)', opacity: 0.7 }}>
              Analyses SPF, DKIM, and DMARC configuration to detect spoofing risks and authentication gaps.
            </div>
          </div>
        )}
      </div>

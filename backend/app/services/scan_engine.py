...
class ScanEngine:
    ...
    async def _run_domain_checks(self, domain: str, platform: str) -> None:
        ...
        if platform == "Google Workspace":
            # GWS path: run GWS checks only
            # Lookalike is handled separately per-domain before platform checks run
            gws_checker = GoogleWorkspaceChecker(
                domain=domain,
                access_token=self.gws_access_token,
                skip_mx_routing=self._has_m365,
            )
            results = await asyncio.gather(gws_checker.run(), return_exceptions=True)
            gws_result = results[0] if not isinstance(results[0], Exception) else {"findings": []}
            ...
            # M365: run DNS + bypass checks in parallel
            # Lookalike runs separately per-domain before platform checks
            domain_coros = [
    ...
    async def run(self) -> Dict:
        ...
        # ── Step 3: Run lookalike detection once per domain ───────────────────
        # Deliberately separate from platform checks to avoid duplication
        # in dual-platform mode where the same domain runs through both paths
        lookalike_tasks = [self._check_lookalike_domains(d) for d in domains]
        await asyncio.gather(*lookalike_tasks, return_exceptions=True)

        # ── Step 4: Run platform-specific domain checks ────────────────────────
        if self._has_m365 and self._has_gws:
